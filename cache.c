	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  if (CACHE_TAGSET(cp, addr) == cp->last_tagset)
    ...
    // COMPUTER DESIGN: Here we are looking for a cache block.
    // If it is found, we jump to cache_hit (in the same function). 
  ...
  /* cache block not found */
    // COMPUTER DESIGN: If you had a miss in the cache, you must choose a new block to replace
    // Assuming FIFO or LRU, this is where you choose a block to evict.
  /* **MISS** */
  cp->misses++;
  ...
  cp->last_blk = NULL;
    ...
    // COMPUTER DESIGN: If the block which will be evicted is valid.
    // In a L1 with a victim cache, you have to send this block to the victim,
    // using blk_access_fn.
  ...
  if (repl->status & CACHE_BLK_VALID)
	...
	*repl_addr = CACHE_MK_BADDR(cp, repl->tag, set);
    ...
    // COMPUTER DESIGN: If the block is dirty, you must write it back, using
    //the blk_access_fn.  You will need to reuse this function elsewhere. 
    ...
    // COMPUTER DESIGN: At this point, we have completed the write (despite the 
    // fact that no data is actually moved.  We are just faking it here.
    // Now we fill in the repl block with new data, tag, and other new bits.
  /* update block tags */
  repl->tag = tag;
  ...
  if (udata)
    *udata = repl->user_data;
    ...
    // COMPUTER DESIGN: This simulates the miss/hit latencies.
  /* update block status */
  repl->ready = now+lat;
  ...
  if (cp->hsize)
    ...
    // COMPUTER DESIGN: This simulates the miss/hit latencies, also.
  /* return latency of the operation */
  return lat;

    // COMPUTER DESIGN: This is if there is a hit in the cache.
    // If there is a hit in the victim cache, we have to write that 
    // block to the L1 and send the L1 block to the victim (a deconstructed swap).
